...
public class MemberInfo { // 필드와 메소드가 있음
    // 필드 추가하기 : 필드란? 클래스 내부에서 사용하는 변수를 의미 (객체 생성 시 객체 갯수 만큼 생산 됨)
    String name;
    int age;
    char gender;
    int jobs;
    ...
    public void setGender() {
        while(true) {
            ...
            gender = sc.next().charAt(0); // 문자열에서 철번째 문자를 입력 받음
            switch(gender) {
                case 'M':
                case 'm':
                case 'F':
    ...
    public void viewInfo() {
        ...
        System.out.println("성별 : " + genderStr[gerGenderType()]);
