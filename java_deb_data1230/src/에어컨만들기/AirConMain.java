...
public class AirConMain {
    public static void main(String[] args) throws InterruptedException {
        ...
        Scanner sc= new Scanner(System.in); // 에어컨을 켜기 위해 콘솔을 입력 받음
        ...
        if (isON.equalsIgnoreCase("Yes")) {
            ...
            lgAirCon.setAirConState(); // 온도와 바람세기 설정하고 설정에 따라 쿨러나 히터를 켬
            while (true) {
                sleep(1000); // 1초
                elapsedTime++;
                ...
                if (isSetTemp) {
                    if (lgAirCon.isHeater()) lgAirCon.setCurrentTemp(1);
                    if (lgAirCon.isCooler()) lgAirCon.setCurrentTemp(-1);
                    lgAirCon.airConInfo();
                    isSetTemp = false;
                    elapsedTime = 0;
                }

                if (lgAirCon.getCurrentTemp() == lgAirCon.getSetTemp()) {
