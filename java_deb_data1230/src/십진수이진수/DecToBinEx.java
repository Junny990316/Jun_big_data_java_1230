...
// 배열을 사용하지 않고 정수 타입 변수를 이용해 변환, 문자열을 사용 금지
public class DecToBinEx {
    ...
    static int decToBin(int dec) { // 10진수를 입력 받아서 2진수로 반환
        int calc; // 계산으로 만들어진 이진수
        int bin = 0; // 최종 결과
        int pos = 1; // 자리수 이동을 위한 포지션 변수
        while (dec != 0) {
    ...
    static int binToDec(int bin) { // 2진수를 입력 받아 10진수로 반환
        int dec = 0; // 최종 결과
        int i = 0; // 인덱스 계산용 변수
